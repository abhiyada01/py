...
#****************************Function*****************
        ...
        def iupdate():
            if (len(E1.get())) != 0 and (len(E2.get())) != 0 and (len(E3.get())) != 0 and (len(E4.get())) != 0 and (len(E5.get())) != 0 and (len(E6.get())) != 0:
                file = ifile()
                file = ifile()
                conn = sl.connect(file)
                c = conn.cursor()
                c.execute('''UPDATE component SET part_name = ?, part_num= ?, pcbname= ?, pcbnum= ?, box_num= ?, description= ?
                          WHERE part_name = ?and part_num= ? and pcbname= ?''',(E1.get(),E2.get(),E3.get(),E4.get(),E5.get(), E6.get(),E1.get(),E2.get(),E3.get()))
        ...
        def isearch():
            if (len(E1.get())) != 0 or (len(E2.get())) != 0 or (len(E3.get())) != 0 or (len(E4.get())) != 0 or (len(E5.get())) != 0 or (len(E6.get())) != 0:
                ...
                conn = sl.connect(file)
                c = conn.cursor()
                c.execute('SELECT * FROM component WHERE (part_name = ? OR part_num= ? OR pcbname= ? OR pcbnum= ? OR box_num= ?)',(E1.get(),E2.get(),E3.get(),E4.get(),E5.get()))
                data = c.fetchall()
                iclear()
                com_list.delete(*com_list.get_children())
                for row in data:
        ...
        com_list = ttk.Treeview(listframe,columns= (1,2,3,4,5,6), show = 'headings',height = 20, yscrollcommand = scroll.set)
